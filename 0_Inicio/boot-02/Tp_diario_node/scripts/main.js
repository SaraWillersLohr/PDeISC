...
import { validarNombre, validarEmail, calcularEdad, validarEdad } from '../modules/validaciones.js';
import { cargarIndicadores } from '../modules/indicadores.js';
import { iniciarTicker } from '../modules/ticker.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnMenuMobile) {
        btnMenuMobile.addEventListener('click', () => {
            setTimeout(() => {
                ...
                iconMenu.classList.replace(isExpanded ? 'bi-list' : 'bi-x-lg', isExpanded ? 'bi-x-lg' : 'bi-list');
    ...
    if (contactoForm) {
        ...
        inputs.nombre.addEventListener('input', () => updateError('nombre', validarNombre(inputs.nombre.value)));
        inputs.email.addEventListener('input', () => updateError('email', validarEmail(inputs.email.value)));
        ...
        inputs.asunto.addEventListener('change', function() {
            ...
            if (data) {
                ...
                inputs.dynamicSelect.innerHTML = '<option value="" disabled selected>Seleccioná una opción</option>';
        ...
        contactoForm.addEventListener('submit', function(e) {
            ...
            if (inputs.asunto.value && !inputs.dynamicSelect.value) updateError('dynamic', "Campo obligatorio.");

            if (errNombre || errEmail || errEdad || !inputs.asunto.value || (inputs.asunto.value && !inputs.dynamicSelect.value)) {
                return;
            }
