...
/** Normaliza los formatos de correo usados por las estrategias y sus respuestas nativas. */
function extractEmail(profile: {
  email?: string;
  emails?: Array<{ value?: string }>;
  _json?: { email?: string; data?: { email?: string } };
  data?: { email?: string };
}): string | null {
  const email =
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "no se pudo validar la cuenta social";
  ...
  if (isSocialProviderEnabled("google")) {
    passport.use(
      new GoogleStrategy(
        ...
        verifySocialProfile("google") as any,
      ),
    );
  }
  if (isSocialProviderEnabled("facebook")) {
    passport.use(
      new FacebookStrategy(
        ...
        verifySocialProfile("facebook") as any,
      ),
    );
  }
      ...
      new DiscordStrategy(
        {
          ...
          callbackURL: `${apiUrl}/api/auth/discord/callback`,
          scope: ["identify", "email"],
        },
        verifySocialProfile("discord"),
      ),
    );
  }
      ...
      new TwitchStrategy(
        {
          ...
          callbackURL: `${apiUrl}/api/auth/twitch/callback`,
          scope: "user:read:email",
        },
        verifySocialProfile("twitch"),
      ),
    );
  }
      ...
      new TwitterStrategy(
        {
          ...
          callbackURL: `${apiUrl}/api/auth/twitter/callback`,
          userProfileURL:
            "https://api.twitter.com/1.1/account/verify_credentials.json",
          includeEmail: true,
        },
        verifySocialProfile("twitter"),
      ),
    );
  }
}
