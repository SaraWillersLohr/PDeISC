...
function renderizarTabla(alumnos) {
  ...
  if (alumnos.length === 0) {
    tbody.innerHTML = '<tr><td colspan="4" class="mensaje-vacio">No se encontraron alumnos.</td></tr>';
    return;
  }
...
function filtrarAlumnos(termino) {
  ...
  const filtrados = alumnosCompletos.filter(alumno =>
...
async function cargarConFetch() {
  ...
  try {
    ...
    actualizarExplicacion(`Fetch obtuvo ${datos.length} alumno(s) de la API. Los datos vienen de MySQL a través del Proyecto A.`);
  } catch (error) {
    actualizarExplicacion('No se pudo conectar con la API. Asegurate de que el Proyecto A esté activo en el puerto 3000.');
  }
}
...
async function cargarConAxios() {
  ...
  try {
    ...
    actualizarExplicacion(`Axios obtuvo ${respuesta.data.length} alumno(s) de la API. Axios convierte el JSON automáticamente en respuesta.data.`);
  } catch (error) {
    actualizarExplicacion('No se pudo conectar con la API. Asegurate de que el Proyecto A esté activo en el puerto 3000.');
  }
}
...
function configurarBotones() {
  document.getElementById('btn-cargar-fetch').addEventListener('click', cargarConFetch);
  document.getElementById('btn-cargar-axios').addEventListener('click', cargarConAxios);
}
