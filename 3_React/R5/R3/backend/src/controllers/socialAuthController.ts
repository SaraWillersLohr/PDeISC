...
import {
  createSocialTicket,
  consumeSocialTicket,
} from "../services/socialTicketService";
...
const frontendUrl =
  process.env.SOCIAL_AUTH_FRONTEND_URL ?? "http://localhost:5173";
...
function redirectWithError(res: Response, message: string): void {
  res.redirect(
    `${frontendUrl}/login?socialError=${encodeURIComponent(message)}`,
  );
}
...
export function startSocialLogin(
  ...
  if (!isSocialProviderEnabled(provider))
    return redirectWithError(res, "este proveedor aún no está configurado");
  ...
  (passport.authenticate as any)(provider, {
    session: false,
    state: true,
...
export function completeSocialLogin(
  req: Request,
  res: Response,
  next: (error?: unknown) => void,
): void {
  ...
  if (!provider || !isSocialProviderEnabled(provider))
    return redirectWithError(res, "proveedor no disponible");

  passport.authenticate(
    provider,
    { session: false, failureRedirect: undefined },
    (
      error: Error | null,
      user: UsuarioPublico | false,
      info?: { message?: string },
    ) => {
      if (error || !user)
        return redirectWithError(
          res,
          info?.message ?? "no se pudo iniciar sesión con la cuenta social",
        );
      const code = createSocialTicket(user, req.session.rememberMe === true);
      req.session.destroy(() => undefined);
      res.redirect(
        `${frontendUrl}/login?socialCode=${encodeURIComponent(code)}`,
      );
    },
  )(req, res, next);
}
...
export function exchangeSocialCode(req: Request, res: Response): void {
  ...
  if (!ticket) {
    res
      ...
      .json({
        success: false,
        message: "el código de acceso social es inválido o venció",
      });
    return;
  }
  ...
  res.json({
    success: true,
    message: "sesión social iniciada correctamente",
    data: { token, usuario: ticket.usuario, rememberMe: ticket.rememberMe },
  });
}
