...
import { validatePetText, validateAgeWeight, validateEmail } from "../modules/validations.js";
...
const validateForm = () => {
    ...
    const isAgeWeightValid = validateAgeWeight(form.especie.value, form.edad.value, form.peso.value).valid;
    const isTerms = termsCheck.checked;

    submitBtn.disabled = !(isPetNameValid && isRazaValid && isDuenoValid && isEmailValid && isAgeWeightValid && isTerms);
};
...
form.addEventListener("input", (e) => {
    ...
    if (input.id === "nombreMascota" || input.id === "raza" || input.id === "nombreDueno") {
        result = validatePetText(input.value);
    } else if (input.id === "email") {
        result = validateEmail(input.value);
    } else if (input.id === "edad" || input.id === "peso" || input.id === "especie") {
        // Chequeo que los datos ingresados sean correctos para edad y peso
        result = validateAgeWeight(form.especie.value, form.edad.value, form.peso.value);
        ...
        if (input.id === "especie") {
            ...
            showFeedback(document.getElementById("peso"), result.field === "peso" ? result : {valid: true});
            validateForm();
            return;
        }
