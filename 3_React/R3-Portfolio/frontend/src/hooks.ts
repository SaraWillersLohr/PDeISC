...
export function usePortfolio(){ const [data,setData]=useState<Portfolio|null>(null),[error,setError]=useState(''); const load=useCallback(async()=>{try{const response=await fetch('/api/portfolio');if(!response.ok)throw new Error();setData(await response.json())}catch{setError('No pudimos cargar el contenido. Verificá que la API y MariaDB estén iniciadas.')}},[]);useEffect(()=>{load()},[load]); return {data,error,reload:load}; }
